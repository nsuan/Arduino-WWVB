...
#define ledBitPin    LED_BUILTIN         // LED bit value indicator digital output
#define ledRxPin     LED_BUILTIN        // LED indicator digital output
//Constants
...
boolean displayUTC = 1;             // change to 1 to display UTC
...
void wwvbInit() {
  ...
  //Set PPS output pin and make sure its low
...
void scanSignal(void){ 
   ...
   if (wwvbSignalState == 1) {             // see if receiver input signal is still high
    ...
    if  (bitCount > 65) { // bad decode-frame reject
          digitalWrite(ledFramePin, HIGH);        
          markCount  = 0;                         // bad start of frame set mark count to zero 
          prevMark   = 0;                         // clear previous to restart frame
          bitCount   = 1;                         // set bit count to one
